            ...
            try:
                ...
                if not quantity_str.isdigit() or int(quantity_str) < 0:
                    error_messages.append(
                        f"Quantidade inválida inserida para o produto {produto.nome} ({produto.marca}). Apenas números inteiros são permitidos."
                    )
                    continue
    ...
    if request.method == "POST":
        ...
        for product_id, quantity_str in zip(product_ids, quantities):
            try:
                ...
                if quantity < 0:
                    error_messages.append(
                        f"A quantidade para adicionar não pode ser negativa para o produto {produto.nome} ({produto.marca})."
                    )
                    continue
...
def home(request):
    ...
    data["valor_total_estoque"] = (
        Produto.objects.aggregate(total=Sum(F("valor") * F("quantidade")))["total"] or 0
    )  # Define 0 como padrão caso esteja vazio
...
def generate_report(request):
    ...
    try:
        ...
        vendas = (
            Venda.objects.filter(data_venda__range=[start_date_utc, end_date_utc])
            .annotate(
        ...
        )  # Excluir vendas com quantidade zero
...
def generate_report(request):
    ...
    return render(
        ...
        "report.html",
        {
            ...
            "no_data_message": no_data_message,
            "data_geracao": data_geracao,
            "total_vendas": f"Tipos de produtos vendidos: {len(vendas)}"
            if group_products
            else f"Total de vendas no período: {len(vendas) if isinstance(vendas, list) else vendas.count()}",
        },
    )
